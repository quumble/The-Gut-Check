# The Gut Check — Test 4C frozen response codebook

**Frozen before Test 4C collection.**

This codebook was developed after inspection of Test 4 and Test 4B. It is therefore prospective for Test 4C only. It must not be described as preregistered for earlier runs.
...
- Code the response as written, not what the coder thinks Claude "really means."
- Do not use output-token length to decide stance.
- Do not use condition identity as evidence for stance.
- If practical, randomize/blind condition labels during coding.
- Preserve the first-pass code sheet.
- Any later recoding rule must be documented as a deviation; do not silently rewrite this codebook after seeing Test 4C.
